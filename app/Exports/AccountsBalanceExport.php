...
class AccountsBalanceExport implements FromCollection, WithHeadings, WithMapping, WithEvents // Ավելացված է WithEvents-ը
{
    public function __construct(
        protected ?string $from = null,
        protected ?string $to = null,
...
//    }
    ...
    public function collection(): Collection
    {
        ...
        // Ֆունկցիա՝ որոշելու համար, թե արդյոք դեբետն ավելացնող նշանն է (Active, Expense, Contra)
        $isDebitPositive = function (?string $type): bool {
            return in_array($type, ['active', 'expense', 'off_balance', 'contra_asset', 'contra']);
        };
        ...
        $accBalance = []; // [account_id => ['balance_amd' => balance, 'type' => type]]
        ...
        foreach ($accById as $id => $acc) {
            ...
            if ($isDebitPositive($acc->type)) {
                $bal = $d - $c; // Active/Expense/Off-balance: Դեբետ - Կրեդիտ
            } else {
                $bal = $c - $d; // Passive/Equity/Income: Կրեդիտ - Դեբետ
            }
        ...
        foreach ($accBalance as $id => $data) {
            $acc = $accById[$id];
            $code = (string)$acc->code;
            $name = (string)$acc->name;
            $amount = $data['balance_amd'];
            $type = $data['type'];
            ...
            if (!isset($base5Sums[$base5])) {
                $base5Sums[$base5] = ['amount' => 0.0, 'type' => $type];
            }
            $base5Sums[$base5]['amount'] += $amount;
        ...
        foreach ($base5Sums as $code5 => $data) {
            $amount = $data['amount'];
            if (abs($amount) < 0.000001) {
                continue;
            }
            $base5Acc = $accById->firstWhere('code', $code5);
            $type = $base5Acc ? $base5Acc->type : '';
    ...
    public function map($row): array
    {
        ...
        $isDebitPositive = in_array($type, ['active', 'expense', 'off_balance', 'contra_asset', 'contra']);

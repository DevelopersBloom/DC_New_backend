...
class AdminController extends Controller
{
    ...
    public function setPawnshop(Request $request)
    {
        $request->validate([
            'pawnshop_id' => 'required|integer|in:1,2',
        ]);

        $user = Auth::user();
        $user->pawnshop_id = $request->input('pawnshop_id');
        $user->save();
    ...
    public function editPawnshop($id)
    {
        ...
        if(!$pawnshop) {
            return response()->json(['error' => 'Pawnshop not found'], 404);
        }
        return response()->json([
            'pawnshop' => $pawnshop
        ]);

...
class DocumentJournalController
{
    ...
    public function txVsJournalDiff(Request $request)
    {
        ...
        // 4. NULL transactionable_type (NULL != X → NULL in SQL, ուստի առանձին query)
        $nullTypeRows = DB::table('transactions')
            ...
            ->select([
                ...
                'amount_amd as tx_amount', 'comment as tx_comment', 'transactionable_id',
            ])
            ->orderBy('date')->get();
        ...
        return response()->json([
            ...
            'null_type_transactions'       => $nullType,
            'journals_without_transaction' => $journalNoTx,
        ]);
    }
}

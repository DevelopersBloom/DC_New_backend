...
//    }
    ...
    public function store(StoreReminderOrderRequest $request): JsonResponse
    {
        ...
        $debitAccount  = ChartOfAccount::find($reminderOrder->debit_account_id);
        $creditAccount = ChartOfAccount::find($reminderOrder->credit_account_id);

        $documentType = DocumentJournal::REMINDER_ORDER_TYPE;

        if ($debitAccount && str_starts_with($debitAccount->code, '8')) {
            $documentType = DocumentJournal::OFF_BALANCE_INCOMING;
        } elseif ($creditAccount && str_starts_with($creditAccount->code, '8')) {
            $documentType = DocumentJournal::OFF_BALANCE_OUTGOING;
        }
        ...
        $debitPartnerCode  = $debitPartner
            ? ($debitPartner->type === 'individual'
                ? ($debitPartner->social_card_number ?? null)
                : ($debitPartner->tax_number ?? null))
            : null;

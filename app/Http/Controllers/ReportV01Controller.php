...
class ReportV01Controller extends Controller
{
    ...
    public function __invoke(Request $request): Response|BinaryFileResponse
    {
        ...
        if ($rows->isEmpty()) {
            ...
            foreach ($rows as $row) {
                ...
                // Since transformToReport1 now only returns 5-digit numeric codes (or 52000),
                // this check should always pass for numeric treatment.
                if (preg_match('/^\d+$/', $code)) {
                    $sheet->setCellValueExplicitByColumnAndRow(1, $currentRow, (float)$code, DataType::TYPE_NUMERIC);
                } else {
                    $sheet->setCellValueExplicitByColumnAndRow(1, $currentRow, $code, DataType::TYPE_STRING);
    ...
    protected function transformToReport1($rows)
    {
        ...
        $grouped = $filteredRows->groupBy(fn($r) => preg_match('/^\d{5}/', (string)$r->code, $m) ? $m[0] : (string)$r->code);
        ...
        $sum6 = $this->sumByPrefix($filteredRows, '6', $sumFields);
        $sum7 = $this->sumByPrefix($filteredRows, '7', $sumFields);

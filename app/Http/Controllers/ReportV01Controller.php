...
//
//        if ($rows->isEmpty()) {
//            $sheet->setCellValueExplicit("A{$currentRow}", '', DataType::TYPE_STRING);
//        } else {
//            foreach ($rows as $row) {
//                $code = (string)$row->code;
//
...
class ReportV01Controller extends Controller
{
    ...
    protected function transformToReport1($rows)
    {
        ...
        $grouped = $rows->groupBy(fn($r) => preg_match('/^\d{5}/', (string)$r->code, $m)
            ? $m[0]
            : (string)$r->code
        );
        ...
        $final = $baseAggregates
            ->merge($letteredNormalized)
            ->sortBy('code')
            ->values();
        ...
        $hasNonZero52000 = collect($net52000)->contains(fn($v) => abs($v) > 0);

        $existingKey = $finalNo67->search(fn($r) => (string)$r->code === '52000');

        if ($hasNonZero52000) {
            if ($existingKey !== false) {
                $r = $finalNo67[$existingKey];
                foreach ($sumFields as $f) {
                    $r->{$f} = $net52000[$f];
                }
                $finalNo67[$existingKey] = $r;
            } else {
                $finalNo67->push((object)array_merge(['code' => '52000'], $net52000));
            }
        } else {
            if ($existingKey !== false) {
                $finalNo67->forget($existingKey);
                $finalNo67 = $finalNo67->values();
            }
        }

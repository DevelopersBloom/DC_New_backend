...
// Ներմուծում ենք լրացուցիչ մոդելները
use App\Models\Client;
use Carbon\Carbon;
// Ենթադրենք ունեք ContractTrait-ը և այն ներմուծված է
    ...
    public function handle(EffectiveRateService $effectiveRateService)
    {
        ...
        foreach ($activeContracts as $contract) {
            ...
            try {
                ...
                $effectiveRate = $contract->effective_daily_rate;

                $calculatedInterest = intval(ceil($interestRate * $amount * 0.01 / 10) * 10);
                $calculatedEffectiveInterest = intval(ceil($effectiveRate * $amount * 0.01 / 10) * 10);
                ...
                if ($calculatedEffectiveInterest > 0) {
                    $journalDocEffective = DocumentJournal::create([
                        'date'                 => $date,
                        'document_number'      => $nextDocNum,
                        'document_type'        => $documentTypeEffective,
                        'amount_amd'           => $calculatedEffectiveInterest,
                        'partner_id'           => $debetPartnerId,
                        'credit_partner_id'    => $creditPartnerId,
                        'comment'              => 'Daily effective rate calculation for contract #' . $contractId,
                    ...
                    Log::info("Daily Effective Rate Accrued: {$calculatedEffectiveInterest} AMD for Contract #{$contractId}");
                }

                ...
                if ($effectiveAmount > 0) {
                    ...
                    if ($ruleEffective) {
                        $journalDocEffective = DocumentJournal::create([
                            ...
                            'debit_partner_id' => $ruleEffective->resolveDebitPartnerId($contract) ?? $contract->client_id,
                            'credit_partner_id' => $ruleEffective->resolveCreditPartnerId($contract) ?? null,
                        ...
                        if ($classification && $classification->reserve_percent > 0) {
                            ...
                            if ($ruleReserve && $reserveAmount > 0) {
                                $journalDocReserve = DocumentJournal::create([
                                    ...
                                    'credit_partner_id' => $ruleReserve->resolveCreditPartnerId($contract) ?? $contract->client_id,
                ...
                if ($calculatedInterest > 0) {
                    ...
                    if ($ruleInterest) {
                        $journalDocInterest = DocumentJournal::create([
                            ...
                            'debit_partner_id' => $ruleInterest->resolveDebitPartnerId($contract) ?? $contract->client_id,
                            'credit_partner_id' => $ruleInterest->resolveCreditPartnerId($contract) ?? $contract->client_id,
                    ...
                    if ($dailyPenaltyAmount > 0) {
                        ...
                        if ($rulePenalty) {
                            ...
                            $journalDocPenalty = DocumentJournal::create([
                                ...
                                'debit_partner_id' => $rulePenalty->resolveDebitPartnerId($contract) ?? $contract->client_id,
                                'credit_partner_id' => $rulePenalty->resolveCreditPartnerId($contract) ?? null,
        ...
        if ($effectiveDiffAmount > 0) {
            ...
            if ($rule86000) {
                ...
                $doc86000 = DocumentJournal::create([
                    ...
                    'debit_partner_id'  => $rule86000->resolveDebitPartnerId($contract) ?? $contract->client_id,
                    'credit_partner_id' => $rule86000->resolveCreditPartnerId($contract) ?? $contract->client_id,
        ...
        if ($nominalAmount > 0) {
            ...
            if ($rule86001) {
                ...
                $doc86001 = DocumentJournal::create([
                    ...
                    'debit_partner_id'  => $rule86001->resolveDebitPartnerId($contract) ?? $contract->client_id,
                    'credit_partner_id' => $rule86001->resolveCreditPartnerId($contract) ?? $contract->client_id,

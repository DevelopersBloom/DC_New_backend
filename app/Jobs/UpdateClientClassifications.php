...
class UpdateClientClassifications implements ShouldQueue
{
            ...
            ->chunkById(200, function ($clients) use ($service) {
                ...
                foreach ($clients as $client) {
                    try {
                        ...
                        $accountsSum = $this->getClientAccountsBalance($clientId, $targetAccountIds);

                        Log::info("Client #{$clientId} accounts sum (16200NV+16201NI+16200): {$accountsSum}");

                        $existingGeneralReserve = $this->getClientReserveBalance($clientId, $acc16605PC);

                        // Get existing special reserve (16605PS) credited for this client
                        $existingSpecialReserve = $this->getClientReserveBalance($clientId, $acc16605PS);
                        ...
                        if ($newClassificationName === 'loss' && $journal) {
                            ...
                            $balance16200 = $this->getClientAccountBalance($clientId, $acc16200);
                            if ($balance16200 > 0) {
    ...
    private function getClientAccountsBalance(int $clientId, array $accountIds): float
    {
        if (empty($accountIds)) {
            return 0.0;
        }
     ...
     */
    private function getClientAccountBalance(int $clientId, ?int $accountId): float
    {
        if (!$accountId) {
            return 0.0;
        }
    ...
    private function getClientReserveBalance(int $clientId, ?int $accountId): float
    {
        if (!$accountId) {
            return 0.0;
        }

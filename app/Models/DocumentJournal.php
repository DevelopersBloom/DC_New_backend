...
class DocumentJournal extends Model
{
    ...
    // Full-payment flow (createAccountingTransaction) stores the posting-rule
    // key as document_type, so these variants exist alongside the ones above.
    const PAY_MOTHER_AMOUNT_TRANSFER = 'pay_mother_amount';
    const PAY_INTEREST_AMOUNT_TRANSFER = 'pay_interest_amount';
    const PAY_INTEREST_AMOUNT_CASH = 'pay_interest_amount_cash';
    ...
    const PAY_PENALTY_AMOUNT = 'Տուգանքի մարում';
...
dd($this->id,$remainingBalance,$loanAmount,$totalAttraction,$totalRepayment);

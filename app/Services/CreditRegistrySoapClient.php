...
class CreditRegistrySoapClient
{
    ...
    // =========================================================
    // CERT VALIDATION (FIXED + SAFE)
    // =========================================================
    private function validateCertificate(): array
    {
        if (!is_file($this->certPath)) {
            throw new \RuntimeException("❌ Certificate not found: {$this->certPath}");
        }

        if (!is_file($this->keyPath)) {
            throw new \RuntimeException("❌ Private key not found: {$this->keyPath}");
        }

        $rawPem = file_get_contents($this->certPath);

        $cert = openssl_x509_read($rawPem);
        if (!$cert) {
        ...
        if ($now < ($info['validFrom_time_t'] ?? 0)) {
            throw new \RuntimeException("❌ Certificate NOT YET VALID");
        }

        if ($now > ($info['validTo_time_t'] ?? 0)) {
            throw new \RuntimeException("❌ Certificate EXPIRED");
        }

        // KEY CHECK (IMPORTANT FIX)
        $priv = openssl_pkey_get_private('file://' . $this->keyPath, $this->certPassword);
        if (!$priv) {
            throw new \RuntimeException("❌ Private key load failed");
        }

        $pubCert = openssl_pkey_get_public($rawPem);
        if (!$pubCert) {
            throw new \RuntimeException("❌ Public key extraction failed");
        }

        $c1 = openssl_pkey_get_details($pubCert);
        $c2 = openssl_pkey_get_details($priv);

        if (($c1['key'] ?? '') !== ($c2['key'] ?? '')) {
            throw new \RuntimeException("❌ CERT / PRIVATE KEY mismatch");
        }

        echo "Certificate subject : " . ($info['subject']['CN'] ?? '-') . PHP_EOL;
        echo "Valid from         : " . date('Y-m-d H:i:s', $info['validFrom_time_t']) . PHP_EOL;
        echo "Valid to           : " . date('Y-m-d H:i:s', $info['validTo_time_t']) . PHP_EOL;
        echo "✅ Certificate VALID + KEY OK\n\n";
    ...
    private function buildEnvelope(string $action, string $bodyContent): string
    {
        $this->validateCertificate(); // 🔥 IMPORTANT
        ...
        return '<?xml version="1.0" encoding="UTF-8"?>'
            . '<s:Envelope xmlns:s="' . self::SOAP_NS . '" xmlns:a="' . self::WSA_NS . '" xmlns:u="' . self::WSU_NS . '" xmlns:o="' . self::WSSE_NS . '" xmlns:ds="' . self::DSIG_NS . '">'
            . '<s:Header>'
            . '<a:Action u:Id="' . $this->actionId . '">' . self::ACTION_NS . $action . '</a:Action>'
            . '<a:MessageID u:Id="' . $this->messageIdId . '">' . $msgId . '</a:MessageID>'
            . '<a:To u:Id="_to">' . $this->endpoint . '</a:To>'
            . '<o:Security>'
            . '<o:BinarySecurityToken u:Id="' . $this->bstId . '" ValueType="' . self::X509_VALUETYPE . '" EncodingType="' . self::B64_ENCODINGTYPE . '">' . $certB64 . '</o:BinarySecurityToken>'
            . '<u:Timestamp u:Id="_ts"><u:Created>' . $now . '</u:Created><u:Expires>' . $exp . '</u:Expires></u:Timestamp>'
            . '</o:Security>'
            . '</s:Header>'
            . '<s:Body u:Id="_body">' . $bodyContent . '</s:Body>'
            . '</s:Envelope>';
    ...
    // =========================================================
    // SIGN (kept safe, same logic as yours)
    // =========================================================
    private function signEnvelope(string $xml): string
    {
        ...
        foreach ($xpath->query('//*[@u:Id]') as $n) {
            $n->setIdAttributeNS(self::WSU_NS, 'Id', true);
        }

        $ts = $xpath->query('//u:Timestamp')->item(0);
        $body = $xpath->query('//s:Body')->item(0);

        $tsDigest = base64_encode(hash('sha256', $ts->C14N(), true));
        $bodyDigest = base64_encode(hash('sha256', $body->C14N(), true));

        $sig = $dom->createElementNS(self::DSIG_NS, 'ds:Signature');
        $si = $dom->createElementNS(self::DSIG_NS, 'ds:SignedInfo');

        $sig->appendChild($si);

        $sec = $xpath->query('//o:Security')->item(0);
        $sec->appendChild($sig);

        $priv = openssl_pkey_get_private('file://' . $this->keyPath, $this->certPassword);
        openssl_sign($si->C14N(), $raw, $priv, OPENSSL_ALGO_SHA256);

        $sig->appendChild(
            $dom->createElementNS(self::DSIG_NS, 'ds:SignatureValue', base64_encode($raw))
        );

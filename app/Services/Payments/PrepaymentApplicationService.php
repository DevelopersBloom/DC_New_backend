...
class PrepaymentApplicationService
{
    ...
    public function apply(Contract $contract, Prepayment $prepayment, ?string $date = null): void
    {
        ...
        if ($principalAmount > 0 || $partialAmount > 0) {
            $contract->left            = max(0, $contract->left - $principalAmount - $partialAmount);
            $contract->provided_amount = max(0, $contract->provided_amount - $principalAmount - $partialAmount);

//            $rule = $this->getPostingRule('prepayment_apply_principal');
//            $this->postEntry(
//                $date, $docNum, DocumentJournal::PREPAYMENT_APPLY_PRINCIPAL,
//                $principalAmount, 'prepayment_apply_principal',

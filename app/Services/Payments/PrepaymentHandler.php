...
/**
 * Handles the "prepayment" payment mechanism:
 *  - Per-row: pays scheduled interest + principal; registers principal paid before
 *    the due date as a Prepayment record (→ liability account 39920).
 *  - Leftover cash: walks future installments and prepays their principal too.
 */
class PrepaymentHandler
{
     ...
     *
     * Interest is paid first (up to the scheduled interest_payment for this row),
     * then principal (up to the scheduled principal_payment). Principal paid before
     * the due date creates a Prepayment record instead of posting to the loan account.
     */
    public function handle(
        ...
        // ── Before due date → register as prepayment (→ account 39920) ─────
        ...
        if ($due->gt($now) && $paidPrincipal > 0) {
            $this->prepaymentService->createSingle(
                $contract->id, $payment->id, $deal_id, $paidPrincipal, $payment->to_date
            );
     ...
     *
     * Walks future payment rows in ascending date order, reduces loan balance,
     * creates/accumulates a Prepayment record for each row, and writes a
     * PaymentEntry so subsequent handle() calls see the covered principal.
     *
     * @param  int[]  $alreadyProcessedIds  IDs of rows already handled in the main loop (skip them)
     * @return float  Total principal registered as prepayments
     */
    public function applyRemaining(
        ...
        foreach ($futurePayments as $row) {
            ...
            // Reduce loan balance (mirrors handle() behaviour)
            ...
            // Record a partial entry so future handle() calls see the covered principal
            PaymentEntry::create([
                'payment_id'       => $row->id,
            ...
            $this->prepaymentService->createSingle(
                $contract->id, $row->id, $dealId, $toPrepay, $row->to_date
            );
        }

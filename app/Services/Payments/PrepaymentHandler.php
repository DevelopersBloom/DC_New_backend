...
//        }
        ...
        $alreadyPaid = (float)$payment->entries()->sum('amount');
        $balanceAfter = (float)$contract->provided_amount;
        $isComplete = ($alreadyPaid + $totalPaid >= (float)$payment->amount - 0.01);
        ...
        if ($timing === PaymentDateClassifier::SOONER) {
            ...
            $partialAmount = $remainingAmount;
            if ($paidPrincipal > 0 || $deferredInterest > 0 || $partialAmount > 0) {
                $bucketPrincipal = $paidPrincipal; // captured before it's zeroed below
                $this->prepaymentService->createSingle(
                    $contract->id, $payment->id, $deal_id,
                    $bucketPrincipal, $deferredInterest, $partialAmount, $payment->to_date, (bool)$cash
                );
                $prepaymentPrincipal = $bucketPrincipal + $deferredInterest + $partialAmount;
                $paidPrincipal = 0;
                $remainingAmount = 0;
                ...
                PaymentEntry::create([
                    ...
                    'user_id' => auth()->id() ?? 1,
                    'reference' => Str::uuid(),
                    'amount' => $prepaymentPrincipal,
                    'principal_amount' => $bucketPrincipal + $partialAmount,
                    'interest_amount' => $deferredInterest,
                    'penalty_amount' => 0,
                    'balance_before' => $balanceBefore,
                    'balance_after' => $balanceAfter,
                    'document_type' => 'prepayment_payment',
        ...
        $finalPaid = $paidInterest + $paidPrincipal;
        if ($isComplete) {
            $this->recorder->completePayment(
                $payment, $payer, $cash, $contract->id, $deal_id,
                $paidPrincipal, $paidInterest, $date, $balanceBefore, $balanceAfter, $finalPaid
            );
        } else {
            $this->recorder->partiallyCompletePayment(
                $payment, $finalPaid, $deal_id, [],
                $paidPrincipal, $paidInterest, $date, $balanceBefore, $balanceAfter
            );
        }

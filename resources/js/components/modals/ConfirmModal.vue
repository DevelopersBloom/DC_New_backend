<template>
    <fwb-modal v-if="show" @close="closeModal" class="confirm-modal">
        <template #header>
            ...
            <template v-if="type === 'regular'">
                <template v-if="penalty">
                    <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400 mt-2">
                        ...
                        <span v-if="debtSummary"> և <span class="text-blue-500 font-bold">{{makeMoney(debtSummary)}}</span> դրամի չվճարված տոկոսագումար</span>
                    </p>
                ...
                <template v-else>
                    <p v-if="debtSummary" class="text-base leading-relaxed text-gray-500 dark:text-gray-400 mt-2">
                        Դուք ունեք <span class="text-blue-500 font-bold">{{makeMoney(debtSummary)}}</span> դրամի չվճարված տոկոսագումար
                    </p>
                    <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400 mt-2">
            ...
            <template v-else-if="type === 'partial'">
                <template v-if="penalty">
                    ...
                    <table class="text-xs text-left w-full text-gray-900 dark:text-gray-400 payments-table mt-4">
                        ...
                        <tr class="border-b dark:border-gray-900 ">
                            <td class="px-2 py-2 dark:text-white">{{makeMoney(amount)}} <Dram/></td>
                            <td class="px-2 py-2 dark:text-white">{{makeMoney(debtSummary)}} <Dram/></td>
                            <td class="px-2 py-2 dark:text-white">{{makeMoney(penalty)}} <Dram/></td>
                            <td class="px-2 py-2 dark:text-white">{{makeMoney(summary)}} <Dram/></td>
                        </tr>
                        </tbody>
                ...
                <div >
                    <div class="">
                        ...
                        <div class="w-max">
                            ...
                            <div class="flex justify-between gap-4 items-center">
                                <label for="surname" class="text-sm dark:text-gray-300">Հեռախոս</label>
                                <fwb-input size="sm" v-model="anotherPayerForm.phone" autocomplete="off" id="phone" class="w-min mt-1"></fwb-input>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    name: 'ConfirmModal', methods: { makeMoney },
    props: {
        penalty: Number,
        show: Boolean,
        type: {
    ...
    setup(props, ctx) {
        ...
        const summary = computed(() => {
            if(props.type === 'regular'){
                ...
                return props.amount + props.penalty + debtSummary.value
            }
        ...
        const alertConfig = computed(() => {
            ...
            if(props.type === 'regular'){
                if(showAnotherAmount.value && anotherAmount.value){
                    if(props.penalty){
                        if(anotherAmount.value <= props.penalty){
                            ...
                            if(debtSummary.value){
                                if(anotherAmount.value <= debtSummary.value + props.penalty){
                        ...
                        if(debtSummary.value){
                            if(anotherAmount.value <= debtSummary.value){
                                amount = anotherAmount.value
                                return returnRes()
                            }else{
                                let difference = anotherAmount.value - debtSummary.value
                                amount = debtSummary.value + difference % 1000
                                partial = difference - difference % 1000
                                return returnRes()
                            }
                        }else{
                ...
                partial = props.amount
                amount = debtSummary.value
                penalty = props.penalty
                return returnRes()

<template>
    <div class="overflow-x-auto sm:rounded-lg container mx-auto pt-10 p-4">
        <div class="flex items-center">
            <h2 class="text-2xl dark:text-white">Զեղչի հարցումներ</h2>
        ...
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg mt-4">
            <div
                ...
                <div class="flex items-center justify-between flex-1 space-x-4">
                    <h5>
                        <span class="text-gray-500">Բոլոր հարցումները:</span>
                        <span class="dark:text-white">{{allDiscounts}}</span>
                    </h5>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-800 dark:text-gray-300">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3">Զեղչ</th>
                        <th scope="col" class="px-4 py-3">Գրավատուն</th>
                        <th scope="col" class="px-4 py-3">Օգտատեր</th>
                    ...
                    <template v-for="discount in discounts">
                        <tr class="border-b text-sm dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                            <td class="px-4 py-3">{{ discount.contract.id }}</td>
                            <td class="px-4 py-3">{{ makeMoney(discount.amount,true) }}</td>
                            <td class="px-4 py-3">{{ discount.pawnshop.city + '/' + discount.pawnshop.address }}</td>
                            <td class="px-4 py-3">{{ discount.user.name + ' ' + discount.user.surname }}</td>
                            <td class="px-4 py-3">
                                <fwb-button size="xs" tag="router-link" :href="'/payments/' + discount.contract.id"><i class="fa fa-money-bill"></i>Էջ</fwb-button>
                            </td>
                        </tr>
                    </template>
...
export default {
    ...
    setup() {
        ...
        const getListData = (url = '/api/admin/get-discounts') => {
            pageLoading.value = true;
            return getAxios().get(url)
            .then((res) => {
                pageLoading.value = false;
                config.value = res.data.discounts
                discounts.value = res.data.discounts.data
                allDiscounts.value = res.data.discounts.total
                return res.data
            })
        }
        getListData()

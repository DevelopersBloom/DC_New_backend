<template>
    <div class="overflow-x-auto sm:rounded-lg container mx-auto pt-10 p-4">
        <div class="flex items-center">
            <h2 class="text-2xl dark:text-white">Փոփոխել գնահատողի տվյալները</h2>
...
export default {
    ...
    setup() {
        ...
        const submitForm = (e) => {
            e.preventDefault();
            getAxios().post('/api/admin/update-evaluator',{...form.value}).then((res) => {
        ...
        const getInfo = () => {
            ...
            return getAxios().get('/api/admin/edit-evaluator/' + route.params.id)
            .then((res) => {
                pageLoading.value = false;
                form.value = res.data.evaluator

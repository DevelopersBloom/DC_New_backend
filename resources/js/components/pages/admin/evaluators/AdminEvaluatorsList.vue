<template>
    <div class="overflow-x-auto sm:rounded-lg container mx-auto pt-10 p-4">
        <div class="flex items-center">
            <h2 class="text-2xl dark:text-white">Գնահատողների ցուցակ</h2>
        ...
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg mt-4">
            <div
                ...
                <div class="flex items-center justify-between flex-1 space-x-4">
                    <h5>
                        <span class="text-gray-500">Բոլոր Գնահատողները:</span>
                        <span class="dark:text-white">{{allUsers}}</span>
                    </h5>
                    <div>
                        <fwb-button size="sm" pill tag="router-link" href="/admin/evaluators/create"><i class="fa fa-user-plus pr-1"></i>Ավելացնել</fwb-button>
                    </div>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-800 dark:text-gray-300">
                    ...
                    <tr>
                        <th scope="col" class="px-4 py-3">Անուն Ազգանուն</th>
                    ...
                    <tbody>
                    <template v-for="(evaluator,index) in evaluators">
                        <tr class="border-b text-sm dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                            <td class="px-4 py-3">{{ evaluator.full_name }}</td>
                            <td class="px-4 py-3">{{ evaluator.pawnshop.city + '/' + evaluator.pawnshop.address }}</td>
                            <td class="px-4 py-3">{{ evaluator.contracts_count }}</td>
                            <td class="px-4 py-3">
                                <fwb-button size="xs" tag="router-link" :href="'/admin/evaluators/edit/' + evaluator.id" color="yellow"><i class="fa fa-edit"></i>Փոփ.</fwb-button>
                            </td>
                        </tr>
                    </template>
...
export default {
    ...
    setup() {
        ...
        const getListData = (url = '/api/admin/get-evaluators') => {
        ...
        getListData().then((data) => {
            allUsers.value = evaluators.value.length
        })
